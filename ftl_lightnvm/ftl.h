...
#define NUM_WRITE_BUFFERS	NUM_BANKS * 3 /* We assume that there can be two in flight, and the third must wait on one to finish. */

#define DRAM_BYTES_OTHER	((NUM_COPY_BUFFERS + NUM_FTL_BUFFERS + NUM_HIL_BUFFERS + NUM_TEMP_BUFFERS + NUM_WRITE_BUFFERS) * BYTES_PER_PAGE \
+ SCAN_LIST_BYTES)

#define WR_WRITE_BUF_PTR(BANK, BUFID)	(WRITE_BUF_ADDR + \
					(((UINT32)(BANK)) * BYTES_PER_PAGE) + \
					(((UINT32)(BUFID)) * BYTES_PER_PAGE))
...
#define WRITE_BUF_ADDR		(SCAN_LIST_ADDR + SCAN_LIST_BYTES)				// page mapping table
